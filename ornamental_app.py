...
# Convert grid to mesh using meshio
def grid_to_mesh(grid):
    ...
    if len(vertices) > 0:
        cells = [("triangle", np.array([[i, (i + 1) % len(vertices), (i + 2) % len(vertices)] for i in range(len(vertices))]))]
...
if canvas_result.image_data is not None:
    ...
    # Convert to mesh
    vertices, cells = grid_to_mesh(ca_grid)
    mesh = meshio.Mesh(vertices, cells)
    mesh.write("output.obj")
    st.write("3D model saved as output.obj")

    # Optionally display the mesh vertices
    st.write("Generated 3D Mesh Vertices:")
    st.write(vertices)

    with open("output.obj", "rb") as file:
        st.download_button(
